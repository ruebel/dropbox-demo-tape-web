...
const Wrapper = styled.div`
  background-color: ${(p) => p.isExpanded ? p.theme.color.background : p.theme.color.backgroundLight};
  bottom: 0;
...
function MobilePlayer() {
  ...
  const playButton = isPlaying ? (
    <IconButton aria-label="Pause" hideBackground={!isExpanded} hideBorder={!isExpanded} onClick={audio.onPause} size={iconSize + 10}>
      <Pause />
    </IconButton>
  ) : (
    <IconButton
  ...
  const inner = isExpanded ? (
    <Expanded>
      ...
      <InfoWrapper>
        ...
        <Artist>
          {audio.playlist?.data?.data?.artist}
        </Artist>
      </InfoWrapper>
      <Position />
      <Buttons>
